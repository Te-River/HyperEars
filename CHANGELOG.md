...
- **荣耀亲选耳机 X5s Pro（BTV-ME10）**：新增已实机验证的具体型号适配。通过私有 RFCOMM SPP
  `5A 00` 帧协议确认后，发布左右耳与充电盒组件电量（含充电标志）以及降噪、关闭、通透三态。
  协议资料见 `docs/honor-x5s-protocol.md`。
- **协议证据确认流程**：荣耀适配遵循"标准能力起步、有效响应确认"——组件电量与三态模式在
  收到合法协议帧后逐项开放，未获有效响应时保持系统标准回退；模式写入由耳机状态回传确认。

### 修复

- **单耳状态误判**：单耳佩戴时状态帧编码与双耳不同，关闭状态不再被误判为降噪（以电量帧的
  未连接组件识别单耳模式）。
